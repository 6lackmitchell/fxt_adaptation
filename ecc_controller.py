...
class EccController(ClfCbfController):
    ...
    def update_error_bounds(self,t):
        ...
        # rho         = np.sqrt(c2_e/c1_e)
        # Xi          = np.arctan2(rho*self.V0**(1/mu_e),1)
        # tan_arg     = Xi-rho*c1_e/mu_e*np.min([t,self.T_e])
        # self.eta    = np.sqrt(kappa*(1/rho*np.tan(np.max([tan_arg,0])))**(mu_e))
        # self.eta    = np.clip(self.eta,0,np.inf)
