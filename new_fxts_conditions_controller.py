...
class FxtsController(ClfCbfController):
    ...
    def set_initial_conditions(self,**kwargs):
        ...
        self.clf        = None
        self.td         = 0
        self.clf0       = clf(self.x,np.array([0,0]))
    ...
    def update_qp(self,x,t,multiplier=1):
        ...
        try:
            if np.sum(LgV.shape) == 2:
                ...
                self.performance = self.m.addConstr(LfV + LgV[0]*self.decision_variables[0] + LgV[1]*self.decision_variables[1] + np.dot(LdV,theta_V) <= 2*self.clf0*(1 - 1/(2*T_c) - self.t/T_c) - 2*V + self.decision_variables[2])
            else:
                ...
                self.performance = self.m.addConstr(LfV + LgV*self.decision_variables[0] + np.dot(LdV,theta_V) <= 2*self.clf0*(1 - 1/(2*T_c) - self.t/T_c) - 2*V + self.decision_variables[1])
        except:
