...
class EccController(ClfCbfController):
    ...
    def set_initial_conditions(self,**kwargs):
        ...
        # Filtered CBF System
        self.hf         = cbf(self.x,0)
        self.hf_dot     = np.zeros(self.x.shape)
        self.h_phif     = 0
        self.h_phif_dot = 0
        self.h_Phif     = np.zeros((np.dot(self.x.T,partialOmega(x))).shape)
        self.h_Phif_dot = np.zeros((np.dot(self.x.T,partialOmega(x))).shape)
        self.h_P        = np.zeros((np.dot(self.h_Phif.T,self.h_Phif)).shape)
        self.h_Q        = np.zeros(self.phi_hat.shape)
    ...
    def update_filter_2ndOrder(self):
        ...
        self.hf_dot     = self.hf_dot   + (self.dt * xf_2dot(self.x,self.xf,self.xf_dot,k_e))
        self.h_phif_dot = self.h_phif_dot + (self.dt * phif_2dot(self.x,self.u,self.phif,self.phif_dot,k_e))
        self.h_Phif_dot = self.h_Phif_dot + (self.dt * Phif_2dot(regressor(self.x,self.t),self.Phif,self.Phif_dot,k_e))

        self.hf   = self.hf   + (self.dt * self.hf_dot)
        self.h_phif = self.h_phif + (self.dt * self.h_phif_dot)
        self.h_Phif = self.h_Phif + (self.dt * self.h_Phif_dot)
    ...
    def update_qp(self,x,t,multiplier=1):
        ...
        upper     = np.clip(self.phi_hat+self.phi_err_max,self.phi_min,self.phi_max)
        lower     = np.clip(self.phi_hat-self.phi_err_max,self.phi_min,self.phi_max)
        prB       = rcbf(x,t,self.err_max,self.Gamma) + np.min([Omega(x)*upper,Omega(x)*lower])
