...
class SFxTSaController(ClfCbfController):
    @property
    def GamMin(self):
        return np.min(np.linalg.eig(self.Gamma)[0])

    @property
    def kV(self):
        min_sigma = 1
        return min_sigma * np.sqrt(2*self.GamMin)

    @property
    def c1_e(self):
        return c1_e * self.kV**(2-2/mu_e)

    @property
    def c2_e(self):
        return c2_e * self.kV**(2+2/mu_e)

    @property
    def T_e(self):
        return mu_e*(c1_e*self.kV**(-2/mu_e)+c2_e*self.kV**(2/mu_e))/(c1_e*c2_e*self.kV**2)
    ...
    def __init__(self,u_max,mode='Rate'):
    ...
    def set_initial_conditions(self,**kwargs):
        ...
        self.W         = np.zeros(regressor(self.x,self.t).shape)
    ...
    def update_auxiliaries(self):
        ...
        if self.mode == 'Filter':
            ...
            ret = np.min(np.linalg.eig(self.P)[0])
            ...
            Wdot   = -l_e*self.W + regressor(self.x,self.t)

            self.z = self.z + (self.dt * zdot)
            self.W = self.W + (self.dt * Wdot)

            ret = np.min(np.linalg.eig(self.W)[0])
    ...
    def update_unknown_parameter_estimates(self):
        ...
        if self.mode == 'Filter':
            ...
            v    = self.P @ self.theta_hat - self.Q
            M    = -self.P
            ...
            v = np.dot(regressor(self.x,self.t),theta) - np.dot(regressor(self.x,self.t),self.theta_hat)
            M = regressor(self.x,self.t)
        ...
        # Update theta_hat
        self.theta_hat_dot  = self.Gamma @ M.T @ v * (c1_e * norm_v**(-2 / mu_e) + c2_e*norm_v**(2 / mu_e))

        # print("Mode: {}".format(self.mode))
        # print("M: {}".format(M))
        # print("v: {}".format(v))
        # print("That: {}".format(self.theta_hat))
        # print("Tdot: {}".format(self.theta_hat_dot))

        if np.linalg.norm(self.theta_hat_dot) >= tol:
    ...
    def update_error_bounds(self,t):
        ...
        # print("Te: {}".format(self.T_e))

        # Update Max Error Quantities
        kappa       = 2*np.max(self.Gamma)
        rho         = np.sqrt(self.c2_e/self.c1_e)
        Xi          = np.arctan2(rho*self.V0**(1/mu_e),1)
        tan_arg     = Xi-rho*self.c1_e/mu_e*np.min([t,self.T_e])
        eta         = np.sqrt(kappa*(1/rho*np.tan(np.max([tan_arg,0])))**(mu_e))
        self.eta    = np.clip(eta,0,np.max(self.theta_max-self.theta_min))

        # Update etadot
        coeff       = -np.sqrt(np.max(self.Gamma)/(2*rho**mu_e))*c1_e*rho
        self.etadot = coeff * np.tan(np.max([tan_arg,0]))**(mu_e/2 - 1) / np.cos(np.max([tan_arg,0]))**2
        ...
        # Update max theta_tilde
        self.err_max = np.clip(eta,0,self.theta_max-self.theta_min)

        if (t*1e4) % 1e4 == 0:
            print("JOU")
            print("errmax: {}".format(self.err_max))
            print("eta:    {}".format(self.eta))
            print("c1_c:   {}".format(c1_c))
            print("c2_c:   {}".format(c2_c))
