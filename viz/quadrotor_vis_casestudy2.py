...
filename = filepath + 'money_sets/WindEstimation/RATE_WIND.pkl'
...
ax4.plot(t[1:ii],disturb[1:ii,3],label=r'$d(t,x)[0]$',color=c1,linewidth=lwidth+3,dashes=dash)
ax4.plot(t[1:ii],disturb[1:ii,4],label=r'$d(t,x)[1]$',color=c2,linewidth=lwidth+3,dashes=dash)
ax4.plot(t[1:ii],disturb[1:ii,5],label=r'$d(t,x)[2]$',color=c3,linewidth=lwidth+3,dashes=dash)
ax4.plot(t[:ii],np.clip(theta_hat[:ii,0],-thetaMax[0],thetaMax[0]),label=r'$\hat{d}(t,x)[0]$',linewidth=lwidth+1,color=c4)
ax4.plot(t[:ii],np.clip(theta_hat[:ii,1],-thetaMax[1],thetaMax[1]),label=r'$\hat{d}(t,x)[1]$',linewidth=lwidth+1,color=c5)
ax4.plot(t[:ii],np.clip(theta_hat[:ii,2],-thetaMax[2],thetaMax[2]),label=r'$\hat{d}(t,x)[2]$',linewidth=lwidth+1,color=c6)
ax4.plot(T_fixed,theta[0],'gd',label='Fixed-Time',markersize=1)
ax4.plot(T_fixed,theta[0],'gd',markersize=20)
ax4.plot(T_fixed,theta[1],'gd',markersize=20)
ax4.plot(T_fixed,theta[2],'gd',markersize=20)
ax4.legend(fancybox=True,markerscale=15,fontsize=25,loc='upper right')
ax4.set(xlabel='Time (sec)',ylabel='Disturbance',xlim=[-3.5,ii*dt+0.5])#,ylim=[-thetaMax[0]-0.5,thetaMax[0]+0.5])
...
ax4a_inset = inset_axes(ax4,width="100%",height="100%",
                      bbox_to_anchor=(.025, .3, .15, .6),bbox_transform=ax4.transAxes, loc=3)
...
ax4a_inset.plot(t[1:ii],disturb[1:ii,3],label=r'$True (x)$',color=c1,linewidth=lwidth+5,dashes=dash)
ax4a_inset.plot(t[1:ii],disturb[1:ii,4],label=r'$True (y)$',color=c2,linewidth=lwidth+5,dashes=dash)
ax4a_inset.plot(t[1:ii],disturb[1:ii,5],label=r'$True (z)$',color=c3,linewidth=lwidth+5,dashes=dash)
ax4a_inset.plot(t[1:ii],np.clip(theta_hat[1:ii,0],-thetaMax[0],thetaMax[0]),label=r'$\hat\theta _{1,PRO}$',linewidth=lwidth+1,color=c4)
ax4a_inset.plot(t[1:ii],np.clip(theta_hat[1:ii,1],-thetaMax[1],thetaMax[1]),label=r'$\hat\theta _{2,PRO}$',linewidth=lwidth+1,color=c5)
ax4a_inset.plot(t[1:ii],np.clip(theta_hat[1:ii,2],-thetaMax[2],thetaMax[2]),label=r'$\hat\theta _{3,PRO}$',linewidth=lwidth+1,color=c6)
ax4a_inset.xaxis.set_major_locator(MaxNLocator(5))
ax4a_inset.yaxis.set_major_locator(MaxNLocator(2))
ax4a_inset.set_xlim(-0.05,0.15)
ax4a_inset.set_ylim(-1.25,4)
...
ax4a_inset.get_xaxis().set_visible(False)
ax4a_inset.get_yaxis().set_visible(False)
ax4a_inset.grid(True,linestyle='dotted',color='white')
mark_inset(ax4,ax4a_inset,loc1=1,loc2=3,fc="none",ec="0.2",lw=1.5)#,ls="--")
