...
# Haven't actually changed anything here yet
class WangController(ClfCbfController):
    ...
    def update_auxiliaries(self):
        ...
        if self.mode == 'Filter':
            ...
            k      = 5
            zdot   = f(self.x) + np.dot(g(self.x),self.u) + np.dot(regressor(self.x,self.t),self.theta_hat) + k*(self.x - self.z) + np.dot(self.W,self.theta_hat_dot)
            Wdot   = -k*self.W + regressor(self.x,self.t)
