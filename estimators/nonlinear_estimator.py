...
class Estimator():
    ...
    def set_initial_conditions(self,**settings):
        ...
        self.W         = np.zeros(regressor(self.x,self.bigTheta).shape)
        self.xi        = self.e
        ...
        reg_depth      = 3
        reg            = regressor(self.x,self.bigTheta)
    ...
    def update(self,
        ...
        if t == 0:
            return self.bigTheta,self.thetaHat,self.errMax,self.etaTerms,self.Gamma,self.xHat,self.theta,self.minSigma
    ...
    def update_unknown_parameter_estimates(self,law=1):
        ...
        if law == 1:
            ...
            prepreXi = self.Gamma @ regressor(self.x,self.bigTheta).T @ self.e
            self.thetaHatDot = prepreXi + pre @ (self.P.T @ W + self.P.T @ W * (W.T @ W))
    ...
    def update_auxiliaries(self):
        ...
        """
        Wdot   = -Kw * self.W + regressor(self.x,self.bigTheta)
        # xi_dot = -Kw * self.xi
    ...
    def update_observer(self):
        ...
        xHatDot = f_reg(self.x,self.bigTheta) + g(self.x)@self.u + regressor(self.x,self.bigTheta)@self.thetaHat + Kw*self.e + self.W@self.thetaHatDot
